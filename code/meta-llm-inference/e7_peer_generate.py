...
ESSAY_FILE_PATH = './data/asap_data/asap_7.csv'
RUBRICS_PATH = './data/rubrics/7_rubrics.txt'
PROMPT_PATH = './data/rubrics/7_prompt.txt'
PEER_REVIEWS_PATH = './data/peer_data/csv_data/essay_scores_set_7.csv'
SEVERITY_PATH = './data/ranking/ranking/set_7_ranking.txt'
OUTPUT_JSON_PATH = './results/peer_results/output_e7_peer_result.json'
ERROR_IDS_PATH = './results/peer_results/e7_error_ids.txt'
...
def build_system_prompt(rubric_content):
    return f"""
You are a rater for essays written by students from grades 7.
...
{{
  ...
  "scores": {{
    "ideas": {{"rationale": "Your rationale here.", "score": 0}},
    "organization": {{"rationale": "Your rationale here.", "score": 0}},
    "style": {{"rationale": "Your rationale here.", "score": 0}},
...
1. Write a justification for each trait: Ideas, Organization, Style, and Conventions.
2. Assign a score from 0 to 3 for each trait.

Notes: Remember that these essays are written by grade 7 students.
Avoid interpreting the rubric too strictly; consider what is developmentally appropriate for grade 7 students.
...
def evaluate_essay_openrouter(essay_id, essay_set, essay_text, prompt_text, rubric_content, peer_df, severity):
    ...
    try:
        ...
        return {
            "essay_id": essay_id,
            "essay_set": essay_set,
            "scores": {trait: {"score": 0, "rationale": f"Error: {e}"} for trait in ['ideas', 'organization', 'style', 'conventions']}
        }
...
for _, row in df.iterrows():
    ...
    try:
        ...
        if all([
            score.get("rationale", "").startswith("Error")
            for score in evaluation_result.get("scores", {}).values()
        ]):
...
print(f"❗ 오류 발생한 에세이 수: {len(error_ids)}개")
...
try:
    with open(ERROR_IDS_PATH, 'w', encoding='utf-8') as f:
        for eid in error_ids:
            f.write(str(eid) + '\n')
    print(f"⚠️ 오류 ID가 '{ERROR_IDS_PATH}' 파일로 저장되었습니다!")
except Exception as e:
    print(f"❌ 오류 ID 저장 실패: {e}")
