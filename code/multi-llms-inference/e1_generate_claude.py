...
# 데이터 경로
# ────────────────────────────────────────────────
ESSAY_FILE_PATH = './data/asap_data/asap_1.csv'
RUBRICS_PATH = './data/rubrics/1_rubrics.txt'
PROMPT_PATH = './data/rubrics/1_prompt.txt'
OUTPUT_JSON_PATH = './results/results1/output_e1_results_emt_claude.json'

TARGET_IDS = [
    25, 33, 34, 42, 47, 49, 52, 62, 63, 99, 123, 135, 192, 234, 235,
    249, 262, 263, 285, 363, 372, 381, 383, 404, 409, 418, 421, 424,
    430, 432, 437, 442, 448, 449, 452, 453, 456, 457, 459, 461, 463,
    465, 467, 468, 473, 474, 486, 609, 690, 692, 693, 696, 732, 758,
    759, 780, 863, 944, 960
]
...
# ────────────────────────────────────────────────
API_CALL_DELAY = 1  
# OPENROUTER_API_KEY = os.environ.get("OPENROUTER_API_KEY", "your key") ###### 여기에 API Key
...
def evaluate_essay_openrouter(essay_id, essay_set, essay_text, prompt_text, rubric_content):
    ...
    try:
        ...
        return {
            "essay_id": essay_id,
            "essay_set": essay_set,
            "scores": {trait: {"score": 0, "rationale": f"Error: {e}"} for trait in ['ideas', 'organization', 'word choice', 'sentence fluency', 'conventions']}
        }
...
try:
    ...
    df = df[df['essay_id'].isin(TARGET_IDS)] # <- 여기 추가함 한줄!!
