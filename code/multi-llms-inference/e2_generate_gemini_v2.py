...
# ────────────────────────────────────────────────
# tee‑logger: copy stdout/stderr to both terminal and a log file
# ────────────────────────────────────────────────
LOG_DIR = "./logs"  # log files will be created here
os.makedirs(LOG_DIR, exist_ok=True)
log_path = os.path.join(LOG_DIR, f"{datetime.now():%Y%m%d_%H%M%S}.log")

class _Tee:
    """Duplicate writes to multiple streams (terminal + file)."""
...
OUTPUT_JSON_PATH = './results/results2/output_e2_re2_results_gemini___4012.json'
...
def evaluate_essay_openrouter(essay_id, essay_set, essay_text, prompt_text, rubric_content):
    ...
    try:
        ...
        return {
            "essay_id": essay_id,
            "essay_set": essay_set,
            "scores": {trait: {"score": 0, "rationale": f"Error: {e}"} for trait in ['content', 'organization', 'word choice', 'sentence fluency', 'conventions']}
        }
